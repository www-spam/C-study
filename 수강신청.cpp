...
public:
	...
	int array = 0;
	string lecture[3] = { "C++ 프로그래밍", "Linux 프로그래밍", "리버싱" }; //기존강의 목록 배열
	string lecture_n[3] = { "0001", "0002", "0003" }; //기존 학수번호 배열
	string num[3] = {}; //추가한 학수번호를 강의명으로 담을 배열
	string num2; //학수번호 입력받는 변수
	...
	void bag_course() //7. bag_course() : 2번 강의 담기 기능 함수
	{
		...
		for (int i = 0; i < 3; i++)
		{                                 
			if (lecture_n[i] == num2) //기존학수번호 배열과 입력한 학수번호가 옳은지 비교. 
			{
				for (int h = 0; h < 3; h++)
				{
					if (num[h] == lecture[i])//기존 강의명과 my장바구니 비교.  장바구니에는 
					{
